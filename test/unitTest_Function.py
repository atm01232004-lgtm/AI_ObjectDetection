...
db = SQLAlchemy(app)
# async_mode='threading' là quan trọng để chạy trên Windows/PyCharm mượt mà
...
def handle_frame(data):
    ...
    try:
        ...
        # B. Chạy AI (Chỉ chạy 1 lần duy nhất ở đây)
        ...
        # C. So sánh với Mục tiêu (Targets) để báo động
        targets = Target.query.all()
        is_alert = False
        for t in targets:
        ...
        if detected_counts and (time.time() - last_save_time > SAVE_COOLDOWN):
            ...
            db.session.add(new_record)
            db.session.commit()
            print(f">>> [DB SAVE] Đã lưu ảnh: {new_record.name}")
